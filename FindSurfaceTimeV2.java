 ...
 * UPDATE: V2 has different answers when you break the limit, like when you try
 * to dive to pressure group Z after diving to pressure group Z. I think
 * I messed it up a little. Further testing required
 */
public class FindSurfaceTimeV2{
    ...
    public static void main(String[] args){
        ...
        int loopNDL[] = {209, 138, 90, 65, 50, 39, 32, 25, 17, 11, 7};
        //END
        
        
        int dives = CheckInput("Input number of dives: ", "Error", 0)-1;
        ...
        int bottomTime = CheckInput("Input bottom time: ", "Error", 1, NDL);
        for (int count=0;count<depths.length;count++){  
            if (depth==depths[count]){
       ...
       for (int coun = 0;coun<dives;coun++){
          ...
          //This will be used to find our new pressure group
           ...
           System.out.print("Your required pressure group is "
           + requiredPressureGroup);
           if (requiredPressureGroup.equals("")){
               System.out.print("A");
           }
           System.out.println("");
           ...
           for(int count = 0; count<alphabet.length;count++){  
               if (oldPressureGroup == alphabet[count]){
                   for (int count2=0; count2<alphabet.length; count2++){
                       ...
                       if (requiredPressureGroup.equals("")){
                           newSurfaceTime = surfaceTimeArray[count][0]+" but it is recommended you go above " + SliceRange
                           (surfaceTimeArray[count][0], 8, 12 )+ "Or not dive to the NDL";
                           /**
